...
public class CodeHelper {
    ...
    public static Integer getCodeByName(@SuppressWarnings("rawtypes") final ArrayList<HashMap> retrieveAllCodes, final String codeName) {
        Integer paymentTypeCodeId = null;
        for (int i = 0; i < retrieveAllCodes.size(); i++) {
            if (retrieveAllCodes.get(i).get("name").toString().equals(codeName) && (Boolean) (retrieveAllCodes.get(i).get("systemDefined"))) {
                paymentTypeCodeId = (Integer) retrieveAllCodes.get(i).get("id");
                Assert.assertNotNull("Code with name PaymentType not found", paymentTypeCodeId);
                break;
            }
        }
        return paymentTypeCodeId;
    }
